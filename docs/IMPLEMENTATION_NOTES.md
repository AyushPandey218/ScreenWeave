...
Earlier milestones may describe superseded behavior. See the root README and docs/evaluation for current capabilities and measurements.
...
Backend diagnostics are available at /dashboard (the API root redirects there). The public dashboard shows process uptime, memory, aggregate request timings and failures, and OCR package availability. Metrics reset on restart; refresh is manual. It never displays uploaded screenshots or private logs.
